...
class da_3d(Ice):
    ...
    def __init__(self):
        ...
        self.observation_error_variance_truth = 0.2
        self.observation_error_variance_estimate = 0.2
        ...
        self.forward_model_method = 'linear interpolation' #"linear interpolation"
        self.observation_method = 'linear interpolation'
        self.dist = distance_periodic(self.grid_obs,self.Lx)
        
        self.assim_method = 1
        print('Instantiating 3DVar Class')
...
#            self.B = gen_cov
    ...
    def correlation_func(self):
        ...
        Ld = self.background_error_decorrelation_length
        c = 2*Ld
        brho = np.copy(self.C_B)
        rho = np.copy(brho)
        for i in range(ni):
            for j in range(nj):
                brho[i,j] = np.exp((-0.5*(x[i]-x[j])**2)/Ld**2)
                z = abs(x[i]-x[j])
                if (z<c):
                    rho[i,j] = -0.25*(z/c)**5 + 0.5*(z/c)**4 + (5./8)*(z/c)**3 - (5./3)*(z/c)**2 + 1
                if(c<= z <=2*c):
                    rho[i,j] = (1./12)*(z/c)**5 - (1./2)*(z/c)**4 + (5./8)*(z/c)**3 + (5./3)*(z/c)**2 - 5*(z/c) + 4 - (2./3)*(c/z)
                if(z>2*c):
                    rho[i,j] = 0
        brho = np.multiply(brho,rho)
        self.C_B = np.copy(brho)
    ...
    def build_H(self):
        ...
        if self.forward_model_method == 'linear interpolation':
            ...
            for i in range(0,self.n_obs):
                ...
                H[i,D[i,:]<self.obs_resolution/2] = 1/float(count)
        else:
    ...
    # Generate the gradient operator
    def build_D(self):
        M = -np.eye(self.Nx-1)
        self.D[1:,:self.Nx-1] += M
        self.D[0,0] = 0;
    ...
    def analysis(self):
     ...
     #       print("select a different error model")
        x0 = self.x_b
        z = np.dot(np.linalg.inv(SQRTM(self.C_B)),(x0-self.x_b))
        x_b = self.x_b
        y = self.y
        sigma_o = self.observation_error_variance_estimate
        sigma_b = self.background_error_variance
        C_B = self.C_B
        H = self.H_observation
        D = self.D
        f = y-np.dot(H,x_b)
        g = np.dot(H,SQRTM(C_B))
        mu = sigma_o/sigma_b
        if self.assim_method == 1:
            res = minimize(func_L2, z, args=(f,g,mu), method=self.optim_method ,jac=grad_func_L2, hess = Hess_func_L2, options={'disp': True})
        elif self.assim_method == 2:
            res = minimize(func_L1, z, args=(f,g,mu,D,C_B,x_b,self.delta), method=self.optim_method ,jac=grad_func_L1, hess = Hess_func_L1, options={'disp': True})
        else:
            print("Invalid assimilation method selected")
        sol = res.x
#        sol = solve(z,f,g,mu,D,C_B,x_b,0.0)
        self.x_a = np.reshape(np.dot(SQRTM(C_B),sol),(len(x_b),1))+x_b
        self.x_b = np.copy(self.x_a)
